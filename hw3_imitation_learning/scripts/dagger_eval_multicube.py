...
Runs policy inference in the multicube goal-conditioned scene. At any time
you (the expert) can press the takeover key to assume manual control. While in
takeover mode every timestep is recorded into a zarr store (same format as
multicube recording in record_teleop_demos.py). Pressing the takeover key
again or ending the episode hands control back to the policy.

Collected data is saved under datasets/raw/multi_cube/dagger/ and can later be
merged with the original demonstrations for retraining.
    ...
    *,
    ...
    while step < max_steps or human_control:
        ...
        if human_control:
            ...
            cubes_state = env.get_all_cubes_state()
        ...
        wrong_cube_color = check_wrong_cube_in_bin(env)
        if success:
        ...
        if wrong_cube_color is not None:
            cv2.putText(
                img,
                f"Wrong cube in bin: {wrong_cube_color}",
                (10, 200),
        ...
        cv2.imshow("DAgger Eval Multicube", img)
...
def main():
    ...
    if not args.headless:
        cv2.namedWindow("DAgger Eval Multicube", cv2.WINDOW_AUTOSIZE)
