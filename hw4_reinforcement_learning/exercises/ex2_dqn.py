...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        # deque automatically evicts the oldest entry when capacity is exceeded.
...
class QNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # Architecture: state -> fc1 -> ReLU -> fc2 -> Q(s, a) for all actions a
        x = F.relu(self.fc1(x))
        return self.fc2(x)
...
class DQN:
    ...
    def take_action(self, state):
        ...
        # With probability epsilon, explore by choosing a uniformly random action.
        # With probability (1 - epsilon), exploit by choosing argmax_a Q(s, a).
    ...
    def update(self, transition_dict):
        ...
        with torch.no_grad():
            ...
            # DQN Bellman target: r + gamma * max_a'{ Q_target(s', a') } * (1 - done)
            # The (1 - done) mask zeroes out the bootstrap term at terminal states.
            # Use the target network for next-state values to stabilise training.
