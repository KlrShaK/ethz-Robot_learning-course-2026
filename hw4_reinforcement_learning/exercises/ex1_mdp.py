        ...
        while True:
            ...
            for s in range(self.env.n_states):
                ...
                for a in range(self.env.n_actions):
                    ...
                    # Bellman expectation: Q(s,a) = sum_s'{ P(s'|s,a) * [r + gamma*V(s')*(1-done)] }
                    for prob, next_state, reward, done in self.env.P[s][a]:
                        qsa += prob * (reward + self.gamma * self.v[next_state] * (not done))
                    # Weight by pi(s,a) so that sum_a gives V(s) = sum_a{ pi(s,a)*Q(s,a) }
            ...
            # stop when the value function has converged (max_diff < theta)
    ...
    def policy_improvement(self):
        ...
        for s in range(self.env.n_states):
            ...
            for a in range(self.env.n_actions):
                ...
                # Q(s,a) = sum_s'{ P(s'|s,a) * [r + gamma*V(s')*(1-done)] }
                for prob, next_state, reward, done in self.env.P[s][a]:
                    qsa += prob * (reward + self.gamma * self.v[next_state] * (not done))
                qsa_list.append(qsa)
...
class ValueIteration:
        ...
        while True:
            ...
            for s in range(self.env.n_states):
                ...
                for a in range(self.env.n_actions):
                    ...
                    # compute all action-values Q(s, a) = sum_s'{ P(s'|s,a) * [r + gamma*V(s')*(1-done)] }
                    for prob, next_state, reward, done in self.env.P[s][a]:
                        qsa += prob * (reward + self.gamma * self.v[next_state] * (not done))
                    qsa_list.append(qsa)
    ...
    def get_policy(self):
        ...
        for s in range(self.env.n_states):
            ...
            for a in range(self.env.n_actions):
                qsa = 0.0
                # compute qsa_list for all actions: Q(s,a) = sum_s'{ P(s'|s,a) * [r + gamma*V(s')*(1-done)] }
                for prob, next_state, reward, done in self.env.P[s][a]:
                    qsa += prob * (reward + self.gamma * self.v[next_state] * (not done))
                qsa_list.append(qsa)
